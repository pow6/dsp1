...
void DFT(int N,int k,int a,struct complex *xn,struct complex *XK);
...
int main()
{
    ...
    int i,a;
    FILE *fpIn,*fpOUT;
    char fileName[24],fileNameOut[24];
    intro();
    ...
    if(mode==1){
        ...
        for(i=0;i<N;i++){
            printf("X%d=%lf+(%lfj)\n",i,XK[i].re,XK[i].im);
            fprintf(fpOUT,"%lf %lf\n",XK[i].re,XK[i].im);
            //fprintf(fpOUT,"%d %lf %lf %lf %lf\n",i,XK[i].re,XK[i].im,sqrt(XK[i].re*XK[i].re+XK[i].im*XK[i].im),atan2(XK[i].im,XK[i].re));
        }
    }else{
        //DFT
        for(i=0;i<N;i++){       
    ...
    printf("処理にかかった時間：%f",(double)(c2-c1)/CLOCKS_PER_SEC);
    fclose(fpIn);
    fclose(fpOUT);
...
void intro(void)
{
    ...
    printf("5.指定したデータがファイルとコマンドラインに出力される\n");
    printf("**************************\n");
}
